...
用法示例:
  python mycode/train.py \
      --data_root /home/zty20020112/workspace/DeepSparse/data/paired \
      --epochs 400 \
      --batch_size 1 \
      --lr 1e-4 \
      --n_views 491 \
...
from datetime import datetime                                       # 时间戳 (日志目录命名)
...
# 将项目根目录加入 sys.path，确保 mycode 包可被导入
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

from mycode.models import SparseViewReconstruction                  # 稀疏视角重建模型
from mycode.dataset import PairedCBCTDataset                        # 配对 CBCT-pCT 数据集
from mycode.losses import ReconstructionLoss                        # 双监督重建损失
...
def train(args):
    ...
    # ---- 日志目录: mylogs/{organ}_{train_views}view_{时间戳}/ ----
    # 例如: mylogs/thorax_6view_20260717_143000/
    ...
    log_dir = os.path.join(                                         # 日志路径
        args.log_dir,                                                # 日志根目录 (默认 mylogs/)
        f'{log_name}_{datetime.now().strftime("%Y%m%d_%H%M%S")}'   # 器官_视角_时间戳
    )
